...
const ApplicationsReceived = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        const res = await API.get('/applications', { params: { role: 'employer' } });
        const apps = res.data;
        ...
        // Attach job info and filter only jobs owned by this employer
        const appsWithJob = apps
          .map(app => ({ ...app, job: jobs.find(j => j.id === app.job_id) }))
          .filter(app => app.job?.user_id === user.id);

        setApplications(appsWithJob);
      } catch (err) {
        console.error(err);
        alert('Failed to fetch applications received');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p className="mt-24 text-center">Loading applications received...</p>;
  if (applications.length === 0) return <p className="mt-24 text-center">No applications received for your jobs yet.</p>;
  ...
  return (
    <div className="mt-24 max-w-6xl mx-auto p-4">
      <h1 className="text-3xl font-mono text-green-700 mb-6 text-center">Applications Received</h1>
